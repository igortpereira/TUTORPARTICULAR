...
@Component({
  selector: 'app-meus-dados-aluno',
  templateUrl: './meus-dados-aluno.page.html',
  styleUrls: ['./meus-dados-aluno.page.scss'],
})
export class MeusDadosAlunoPage implements OnInit {

  senhaVisivel = false;
  aluno: Aluno;
  formGroup: FormGroup;

  constructor(private activatedRoute: ActivatedRoute, private toastController: ToastController, private navController: NavController, private formBuilder: FormBuilder, private alunoService: AlunoService) { 
    ...
    this.formGroup = this.formBuilder.group(
      {
        ...
        'cidade': [this.aluno.cidade, Validators.compose([
          Validators.required
        ])],
    ...
    if(idAluno != null) {
      this.alunoService.buscarPorId(parseInt(idAluno)).then((json)=>{
        ...
        this.formGroup.get('cpf')?.setValue(this.aluno.cpf);
        this.formGroup.get('email')?.setValue(this.aluno.email);
        this.formGroup.get('senha')?.setValue(this.aluno.senha);
        this.formGroup.get('telefone')?.setValue(this.aluno.telefone);
        this.formGroup.get('uf')?.setValue(this.aluno.uf);
        this.formGroup.get('cep')?.setValue(this.aluno.cep);
        this.formGroup.get('cidade')?.setValue(this.aluno.cidade);
        this.formGroup.get('bairro')?.setValue(this.aluno.bairro);
        this.formGroup.get('rua')?.setValue(this.aluno.rua);
        this.formGroup.get('numero')?.setValue(this.aluno.numero);
      }); 
    }
  }
  ...
  salvar() {
    ...
    this.alunoService.salvar(this.aluno).then((json) => {
      ...
      if (this.aluno) {
        ...
        this.navController.navigateBack('/inicio-aluno/' + this.aluno.idAluno);
      } else { 
